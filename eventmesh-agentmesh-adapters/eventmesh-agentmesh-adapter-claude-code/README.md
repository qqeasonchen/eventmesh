...
Claude Code 通过 MCP 协议接入 EventMesh A2A AgentMesh 的桥接服务器。
...
│              │  tools/list/call │  (Python)         │               │  Gateway :10105  │
└──────────────┘                  └──────────────────┘               └──────────────────┘
```

Claude Code 原生支持 MCP 协议。通过这个 MCP Bridge Server，Claude Code 可以：
...
```json
{
  "mcpServers": {
    "eventmesh-agentmesh": {
      "command": "python3",
      "args": [
        "/absolute/path/to/claude_code_mcp_server.py"
      ],
      "env": {
